...
describe("competition tests", () => {
  ...
  test("check buildQuery correctly builds the query", () => {
    ...
    expectedQuery =
      "#competition #jparrot_v2 #uniboswe2021 is:reply #test the cat is on the table";

    expect(competition.buildQuery(hashtag, correctAnswer)).toEqual(
      expectedQuery
    );
  });
  ...
  test("check if searchReplies can correctly access the info needed", async () => {
    ...
    if (search) {
      expect(search.data[0].text).toEqual(expect.stringContaining(expected));
    } else {
  ...
  test("check organizeAnswers correctly organizes the replies correctly using stored blobs - open-ended", () => {
    ...
    expect(realOutput).toEqual(expect.objectContaining(expectedOutput));
  });
